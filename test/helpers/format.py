...
def format_cycle(dut, cycle_id: int, thread_id: Optional[int] = None):
    ...
    for core in dut.cores:
        ...
        if int(dut.thread_count.value) <= int(core.i.value) * int(dut.THREADS_PER_BLOCK.value):
        ...
        for thread in core.core_instance.threads:
            if int(thread.i.value) < int(core.core_instance.thread_count.value):  # if enabled
